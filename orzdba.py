...
def get_options():
    ...
    parser.add_argument('-p', '--pwd', action='store', help='root user password.')
    ...
    if opts.pwd is not None: gl.my_pwd = opts.pwd
...
def main():
    ...
    while (1):
        ...
        if (gl.timeFlag == 1):
            cprint(
                colored(time.strftime("%H:%M:%S", time.localtime()), 'yellow') + colored('|', 'blue', attrs=['bold']),
                end='')
        # (2) Print SysInfo
        get_sysinfo()
