...
def get_options():
    #define args
    parser = argparse.ArgumentParser(description='Created By lubo 454331202@qq.com')
    ...
    if (opts.threads or opts.bytes or opts.rt or opts.com or opts.hit or opts.innodb_rows or opts.innodb_pages or opts.innodb_data or opts.innodb_log or opts.innodb_status):gl.mysql=1
    ...
    if opts.time:
       ...
       gl.headline2 = colored('  time  |','blue',attrs=['bold','underline'])
       gl.timeFlag = 1
       gl.optflag  = 1
    if opts.load:
       gl.headline1 +=colored(' -----load-avg----','blue',attrs=['bold'])
       gl.headline2 +=colored('  1m    5m   15m |','blue',attrs=['bold','underline'])
       gl.my_load = 1
       gl.optflag = 1
    if opts.cpu:
       gl.headline1 +=colored(' ---cpu-usage--- ','blue',attrs=['bold'])
       gl.headline2 +=colored('usr sys idl iow|','blue',attrs=['bold','underline'])
       gl.my_cpu = 1
       gl.optflag = 1
    if opts.swap:
       gl.headline1 += colored('---swap--- ','blue',attrs=['bold'])
       gl.headline2 += colored('   si   so|','blue',attrs=['bold','underline'])
       gl.my_swap = 1
       gl.optflag = 1
    if opts.net:
       gl.headline1 += colored(' -----net(B)----- ','blue',attrs=['bold'])
       gl.headline2 += colored('    recv   send |','blue',attrs=['bold','underline'])
       gl.optsflag = 1
    if opts.disk:
       gl.headline1 += colored('-------------------------io-usage----------------------- ','blue',attrs=['bold'])
       gl.headline2 += colored('   r/s    w/s    rkB/s    wkB/s  queue await svctm \%util|','blue',attrs=['bold','underline'])
       gl.optsflag =1

    if opts.com:
       gl.mysql_headline1 += colored('-----------QPS--TPS----------- ','green','on_blue',attrs=['bold'])
       gl.mysql_headline2 += colored('  ins   upd   del    sel   iud|','green',attrs=['bold','underline'])
       gl.com=gl.optflag=1
    if opts.hit:
       gl.mysql_headline1 +=colored('------Hit%----- ','green','on_blue',attrs=['bold'])
       gl.mysql_headline2 +=colored('    lor     hit|','green',attrs=['bold','underline'])
       gl.innodb_hit=gl.optflag=1
    if opts.innodb_rows:
       gl.mysql_headline1 +=colored('---innodb rows status--- ','green','on_blue',attrs=['bold'])
       gl.mysql_headline2 +=colored('  ins   upd   del   read|','green',attrs=['bold','underline'])
       gl.innodb_rows=gl.optflag=1
    if opts.innodb_pages:
       gl.mysql_headline1 +=colored('---innodb bp pages status-- ','green','on_blue',attrs=['bold'])
       gl.mysql_headline2 += colored('   data   free  dirty flush|','green',attrs=['bold','underline'])
       gl.innodb_pages=gl.optflag=1
    if opts.innodb_data:
       gl.mysql_headline1 += colored('-----innodb data status---- ','green','on_blue',attrs=['bold'])
       gl.mysql_headline2 += colored(' reads writes  read written|','green',attrs=['bold','underline'])
       gl.innodb_data=gl.optflag=1
    if opts.innodb_log:
       gl.mysql_headline1 +=colored('--innodb log-- ','green','on_blue',attrs=['bold'])
       gl.mysql_headline2 += colored('fsyncs written|','green',attrs=['bold','underline'])
       gl.innodb_log=gl.optflag=1
    if opts.innodb_status:
       gl.mysql_headline1 +=colored('  his --log(byte)--  read ---query--- ','green','on_blue',attrs=['bold'])
       gl.mysql_headline2 +=colored(' list uflush  uckpt  view inside  que|','green',attrs=['bold','underline'])
       gl.innodb_status=gl.optflag=1
    if opts.threads:
       gl.mysql_headline1 +=colored('------threads------ ','green','on_blue',attrs=['bold'])
       gl.mysql_headline2 +=colored(' run  con  cre  cac|','green',attrs=['bold','underline'])
       gl.threads=gl.optflag=1
    if opts.bytes:
       gl.mysql_headline1 +=colored('-----bytes---- ','green','on_blue',attrs=['bold'])
       gl.mysql_headline2 +=colored('   recv   send|','green',attrs=['bold','underline'])
       gl.bytes=gl.optflag=1
    if opts.rt:
       gl.mysql_headline1 +=colored('--------tcprstat(us)-------- ','green','on_blue',attrs=['bold'])
       gl.mysql_headline2 +=colored('  count    avg 95-avg 99-avg|','green',attrs=['bold','underline'])
       gl.dbrt=gl.optflag=1
    if opts.port is not None:gl.my_port=opts.port
    if opts.socket is not None:gl.my_socket=opts.socket
    if opts.net is not None:gl.my_net=opts.net
    if opts.disk is not None:gl.my_disk=opts.disk
    if opts.count is not None:gl.count=opts.count
...
def get_mysql_vars():
   #get dbname
   p=subprocess.Popen(gl.mysql_conn+' -e "show databases" |grep -iwvE "information_schema|mysql|test" | tr "\n" "|"',shell=True,stdout=subprocess.PIPE)
   out=p.communicate()[0]
...
def  readCpuInfo():
    ...
    user_diff   = int(sys_cpu2[1]) + int(sys_cpu2[2]) - int(gl.sys_cpu1[1]) - int(gl.sys_cpu1[2])
    system_diff = int(sys_cpu2[3]) + int(sys_cpu2[6]) + int(sys_cpu2[7]) - int(gl.sys_cpu1[3]) - int(gl.sys_cpu1[6]) - int(gl.sys_cpu1[7])
...
def readNetInfo(dev):
    ...
    if net2:
       if gl.net_not_first:
           recv_diff=(float(net2['recv'])-float(gl.net1['recv']))/gl.interval
           send_diff=(float(net2['send'])-float(gl.net1['send']))/gl.interval
       else:
            recv_diff=0
            send_diff=0
...
# sda            0.05    12.44  0.42  7.60     5.67    80.15    21.42     0.04    4.63   0.55   0.44
    ...
    if sys_io2 is None or len(sys_io2)==0:
       return None
    rd_ios     = float(sys_io2[3])  - float(gl.sys_io1[3])   #/* Read I/O operations */
    rd_merges  = float(sys_io2[4])  - float(gl.sys_io1[4])   #/* Reads merged */
    rd_sectors = float(sys_io2[5])  - float(gl.sys_io1[5])   #/* Sectors read */
    rd_ticks   = float(sys_io2[6])  - float(gl.sys_io1[6])   #/* Time in queue + service for read */
    wr_ios     = float(sys_io2[7])  - float(gl.sys_io1[7])   #/* Write I/O operations */
    wr_merges  = float(sys_io2[8])  - float(gl.sys_io1[8])   #/* Writes merged */
    wr_sectors = float(sys_io2[9])  - float(gl.sys_io1[9])   #/* Sectors written */
    wr_ticks   = float(sys_io2[10]) - float(gl.sys_io1[10])  #/* Time in queue + service for write */
    ticks      = float(sys_io2[12]) - float(gl.sys_io1[12])  #/* Time of requests in queue */
    aveq       = float(sys_io2[13]) - float(gl.sys_io1[13])  #/* Average queue length */
...
def get_mysqlstat():
    ...
    if gl.mysql:
       ...
       if gl.mysql_no_first:
          insert_diff =(int(mystat2["Com_insert"]) - int(gl.mystat1["Com_insert"]))/gl.interval
          update_diff =(int(mystat2["Com_update"]) - int(gl.mystat1["Com_update"]))/gl.interval
          delete_diff =(int(mystat2["Com_delete"]) - int(gl.mystat1["Com_delete"]))/gl.interval
          select_diff =(int(mystat2["Com_select"]) - int(gl.mystat1["Com_select"]))/gl.interval
          read_request =(int(mystat2["Innodb_buffer_pool_read_requests"]) - int(gl.mystat1["Innodb_buffer_pool_read_requests"]))/gl.interval
          read         =(int(mystat2["Innodb_buffer_pool_reads"]) - int(gl.mystat1["Innodb_buffer_pool_reads"]))/gl.interval
          innodb_rows_inserted_diff =(int(mystat2["Innodb_rows_inserted"]) - int(gl.mystat1["Innodb_rows_inserted"]))/gl.interval
          innodb_rows_updated_diff  =(int(mystat2["Innodb_rows_updated"])  - int(gl.mystat1["Innodb_rows_updated"]))/gl.interval
          innodb_rows_deleted_diff  =(int(mystat2["Innodb_rows_deleted"])  - int(gl.mystat1["Innodb_rows_deleted"]))/gl.interval
          innodb_rows_read_diff     = (int(mystat2["Innodb_rows_read"])    - int(gl.mystat1["Innodb_rows_read"]))/gl.interval
          innodb_bp_pages_flushed_diff=(int(mystat2["Innodb_buffer_pool_pages_flushed"]) - int(gl.mystat1["Innodb_buffer_pool_pages_flushed"]))/gl.interval
          innodb_data_reads_diff    = (int(mystat2["Innodb_data_reads"])    - int(gl.mystat1["Innodb_data_reads"])     ) /gl.interval
          innodb_data_writes_diff   = (int(mystat2["Innodb_data_writes"])   - int(gl.mystat1["Innodb_data_writes"])    ) /gl.interval
          innodb_data_read_diff     = (int(mystat2["Innodb_data_read"])     - int(gl.mystat1["Innodb_data_read"])      ) /gl.interval
          innodb_data_written_diff  = (int(mystat2["Innodb_data_written"])  - int(gl.mystat1["Innodb_data_written"])   ) /gl.interval
          innodb_os_log_fsyncs_diff = (int(mystat2["Innodb_os_log_fsyncs"]) - int(gl.mystat1["Innodb_os_log_fsyncs"])  ) /gl.interval
          innodb_os_log_written_diff= (int(mystat2["Innodb_os_log_written"])- int(gl.mystat1["Innodb_os_log_written"]) ) /gl.interval
          threads_created_diff      = (int(mystat2["Threads_created"])      - int(gl.mystat1["Threads_created"])      ) /gl.interval
          bytes_received_diff       = (int(mystat2["Bytes_received"])       - int(gl.mystat1["Bytes_received"])       ) /gl.interval
          bytes_sent_diff           = (int(mystat2["Bytes_sent"])          - int(gl.mystat1["Bytes_sent"])           ) /gl.interval
          if gl.com:
...
def get_sysinfo():
    ...
    if gl.my_swap:
        ...
        if swapinfo['pswpin']>0:
            cprint ("{:5d}".format(int(swapinfo['pswpin']/gl.interval)),'red',end='')
        else:
            cprint ("{:5d}".format(0),'white',end='')
        if swapinfo['pswpout']>0:
            cprint ("{:5d}".format(int(swapinfo['pswpout']/gl.interval)),'red',end='')
        else:
            cprint ("{:5d}".format(0),'white',end='')
        cprint ('|','blue',attrs=['bold'],end='')
    ...
    if gl.my_disk is not None:
           deltams = 1000* (cpuinfo['userdiff'] + cpuinfo['systemdiff'] + cpuinfo['idlediff'] +cpuinfo['iowaitdiff'] ) /gl.ncpu /gl.HZ
...
def get_dbrt():
    ...
    try:
       ...
       if pid==0:
          ...
          my_tcprstat='/usr/bin/tcprstat --no-header -t 1 -n 0 -p '+gl.my_port+' -l '+get_ip_address()
          my_tcprstat+= ' >'+gl.tcprstat_dir+'orzdba_tcprstat.'+str(chpid)+'.log'
          #print my_tcprstat
          os.execlp("/bin/sh", "sh", "-c",my_tcprstat)
          sys.exit(0)
       gl.tcprstat_logfile=gl.tcprstat_dir+'orzdba_tcprstat.'+str(pid)+'.log'
       cprint("{:>7.0f}{:>7.0f}{:>7.0f}{:>7.0f}".format(0,0,0,0),'green',end='')
       cprint('|','green',attrs=['bold'],end='')
...
def main():
    ...
    if gl.orz_logfile is not None:
       ...
       if gl.logfile_by_day:
          ...
          logfile=gl.orz_logfile+'.'+time.strftime('%Y%m%d',time.localtime(time.time()))
       else:
    ...
    while(1):
        #-C:Times to exists
        if (gl.count is not None and mycount+1>gl.count):break
        if (gl.orz_logfile is not None and gl.logfile_by_day):
            newlogfile=gl.orz_logfile+'.'+time.strftime('%Y%m%d',time.localtime(time.time()))
